...
func main() {
	...
	for _, j := range paths {
		// Checks data path
		path := filepath.Join(j, "src", "github.com", "SynthBrain", "synhBrain")
	...
	synB.win, err = synB.wmgr.CreateWindow(1200, 900, "SynthBrain", false)//synB.userData.FullScreen
	if err != nil {
		panic(err)
	}
	...
	synB.levelStyle = NewStandardStyle(synB.dataDir)
	...
	//synB.InitLevel(synB.userData.LastLevel)
